...
export async function onRequest(context) {
    ...
    async function fetchContentWithType(targetUrl) {
        ...
        try {
            ...
            // 如果是 M3U8，我们需要读取为文本进行处理
            if (isM3u8Content('', contentType) || targetUrl.includes('.m3u8')) {
                const content = await response.text();
                logDebug(`请求成功 (M3U8): ${targetUrl}, 内容长度: ${content.length}`);
                return { content, contentType, responseHeaders: response.headers, isBinary: false };
            } else {
                // 对于图片等二进制内容，直接返回 body 流
                logDebug(`请求成功 (Binary): ${targetUrl}, Content-Type: ${contentType}`);
                return { content: response.body, contentType, responseHeaders: response.headers, isBinary: true };
            }
    ...
    try {
        ...
        if (!isBinary && isM3u8Content(content, contentType)) {
            ...
            logDebug(`直接返回内容 (Binary: ${isBinary}, Type: ${contentType}): ${targetUrl}`);
